class initnm{
	...
	static iniciar(){
		...
		initnm.#forSimpleInputs((ele,index)=>{
			const chave = 'mascara_identificador'+index;
			const mascaraValor = ele.getAttribute("numascara");
			if(mascaraValor === '') return;
			//fazer os gabaritos
			initnm.#gabaritosMascaras[chave] = initnm.#interpretarMask(mascaraValor);
			ele.addEventListener('input',()=>{
				//mudar o valor do input interpretando a mascara
				const textoInicial = ele.value;
				ele.value = initnm.#aplicarMask(initnm.#gabaritosMascaras[chave],textoInicial);
			});	
		});
		...
		initnm.#forAliasInputs(ele=>{
			...
			ele.addEventListener('input',()=>{
				//mudar o valor do input interpretando a mascara
				const textoInicial = ele.value;
				ele.value = initnm.#aplicarMask(initnm.#gabaritosMascaras[mascaraNome],textoInicial);
			});
		});
	}

	static #forSimpleInputs(fn){
		const inputs = document.querySelectorAll('input[numascara]');
		inputs.forEach(fn);
	}

	static #forAliasInputs(fn){
		const inputs = document.querySelectorAll('input[mascara]');
		inputs.forEach(fn);
	}

	static #aplicarMask(gabarito,texto_pre){//funcionando
	...
	static #interpretarMask(mask){//funcionando
